...
class SegmentationModel(nn.Module):
  ...
  def fit(self, donnees_dict: dict, classes_mapping: dict, augmentations: A.Compose=None,
    ...
    if n_classes == 1:
      if len(classes_pixels_values) != 2:
        raise ValueError("The number of classes the model should output must be equal to the number of classes specified in the classes mapping!")
    else:
  ...
  def predict(self, image: Union[str, np.ndarray], image_size: Union[int, List, Tuple]=None, device="cpu"):
    """
    ...
    if type(image_size) not in [int, list, tuple]:
          raise TypeError("prediction_size must be either an integer or a list of 2 elements or a tuple of two elements")
    ...
    image_tensor = preprocess_input_for_prediction(image=image_array, width=img_width, height=img_height, normalize=True)
